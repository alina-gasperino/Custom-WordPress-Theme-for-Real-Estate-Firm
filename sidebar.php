...
// Get sidebar settings from the Wordpress Customizer.
// $sidebar = AE_Structure::sidebar();

// Overwrite the sidebar for certain theme templates.
/*
if( is_page_template( 'templates/example.php' ) ) {
	$sidebar['type'] = 'sidebar--example';
}
*/

// If the sidebar is set to hidden, do not output it.
// if ( $sidebar['hide'] == false ) : 
    ...
    /* . $sidebar['type'] . ' ' . $sidebar['layout'] . */

    echo "<aside class='sidebar  sidebar_" . $sidebar . " " . $sidebar_smartphone . " " . avia_layout_class( 'sidebar', false )." units' ".avia_markup_helper(array('context' => 'sidebar', 'echo' => false))." itemscope itemtype='http://schema.org/WPSideBar'>";

    /* if ( is_active_sidebar( $sidebar['type'] ) ) : ?>
        ...
        <?php elseif( current_user_can( 'administrator' ) ): ?>
            ...
            <div class="sidebar-no-widgets">
                <p>
                    <?php esc_html_e( 'There are no active widgets for the current page template. Please add some from the dashboard.', 'talk' ); ?>
                </p>
            </div>
