...
if ( !class_exists( 'talk_project_grid' ) )
{
	class talk_project_grid
	{
		static  $grid = 0;
		...
		public function html()
		{
			...
			foreach ($this->entries->posts as $entry)
			{
				...
				$terms = get_the_terms( $entry->ID, 'city' );
				if (is_array($terms)) $city = reset($terms)->name;

				$terms = get_the_terms( $entry->ID, 'neighbourhood' );
				if (is_array($terms)) {
					$neighbourhoods = array();
					foreach ($terms as $term) {
						$neighbourhoods[] = $term->name;
					}
					$neighbourhood = implode(' > ', $neighbourhoods );
				}

				$terms = get_the_terms( $entry->ID, 'developer' );
				if (is_array($terms)) {
					$developers = array();
					foreach ($terms as $term) {
						$developers[] = $term->name;
					}
					$developer = implode(' & ', $developers );
				}
				...
				if ($city && $neighbourhood) {
					$title .= "<p><i class='fa fa-fw fa-map-marker'></i> $city ($neighbourhood)</p>";
				} elseif ($city) {
					$title .= "<p><i class='fa fa-fw fa-map-marker'></i> $city</p>";
				}
				$title .= ($developer) ? "<p><span class='label iconfont' " . av_icon_string('tools') . "></span> $developer</p>" : '';
				// $title .= ($neighbourhood) ? "<p>$neighbourhood</p>" : '';
				// $title .= ($majorintersection) ? "<p><i class='fa fa-fw fa-map-marker'></i> $majorintersection</p>" : '';

				$terms = get_the_terms( $entry->ID, 'occupancy-date' );
				if (is_array($terms)){
					$occupancydate = reset($terms)->name;
					$title .= ($occupancydate) ? "<p><i class='fa fa-fw fa-check-circle'></i> Occupancy: $occupancydate</p>" : '';
				} 
                ...
                else
                {
                    ...
                    $output .= (has_post_thumbnail($the_id)) ?
                    	...
                    	'<img class="placeholder" src="' . get_stylesheet_directory_uri() . '/assets/images/gallery-placeholder-square.png' . '" />';
                    $output .= "</".$link_markup[1].">";
                    $output .= $labels;
                    $output .= '</div>';
