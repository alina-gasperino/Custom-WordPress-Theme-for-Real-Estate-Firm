	...
	class avia_post_grid
	{
		...
		public function html()
		{
			...
			foreach ($this->entries->posts as $entry)
			{
				...
				if (is_array($terms)) $occupancydate = reset($terms)->name;

				$title = '<h5>' . $title . '</h5>';
				$title .= ($city) ? "<p>$city</p>" : '';
				// $title .= ($neighbourhood) ? "<br>$neighbourhood" : '';
				$title .= ($developer) ? "<p>By: $developer</p>" : '';
				$title .= ($neighbourhood) ? "<p>$neighbourhood</p>" : '';
				$title .= ($majorintersection) ? "<p>$majorintersection</p>" : '';
				$title .= ($occupancydate) ? "<p>Occupancy: $occupancydate</p>" : '';
