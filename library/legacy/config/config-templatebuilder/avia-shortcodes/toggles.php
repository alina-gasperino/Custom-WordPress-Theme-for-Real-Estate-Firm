...
if ( !class_exists( 'avia_sc_toggle' ) )
{
    class avia_sc_toggle extends aviaShortcodeTemplate
    {
        ...
        function av_toggle($atts, $content = "", $shortcodename = "")
        {
            ...
            $toggle_atts = shortcode_atts(array('title' => '', 'date' => '', 'tags' => '', 'custom_id' => '', 'custom_markup' =>'', 'anchor' => ''), $atts, 'av_toggle');
            ...
            if ($toggle_atts['anchor']) $output .= '<a name="'. $toggle_atts['anchor'] . '"></a>';
            $output .= '    <div class="single_toggle" '.$this->create_tag_string($toggle_atts['tags'], $toggle_atts).' >';
            $output .= '        <p data-fake-id="#'.$toggle_atts['custom_id'].'" class="clearfix toggler '.$titleClass.'" '.$markup_title.'>'.$toggle_atts['title'] . '<span class="date">' . $toggle_atts['date'] . '</span>' .'<span class="toggle_icon">';
