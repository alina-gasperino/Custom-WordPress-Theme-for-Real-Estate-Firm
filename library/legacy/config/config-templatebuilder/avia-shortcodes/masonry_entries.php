			...
			foreach($this->loop as $entry)
			{
				...
				if($this->atts['caption_elements'] != 'none' || !empty($text_add))
				{
					...
					if(strpos($this->atts['caption_elements'], 'title') !== false){
						...
						if (custom_cat_text('neighbourhood', $ID) && custom_cat_text('city', $ID)) {
							$items .=	"<p>" . custom_cat_text('neighbourhood', $ID) . ", " . custom_cat_text('city', $ID) . "</p>";
						} elseif (custom_cat_text('city', $ID)) {
							$items .=	"<p>" . custom_cat_text('city', $ID) . "</p>";
						}

						$pricedfrom = project_pricedfrom($ID);
						$pricedto = project_pricedto($ID);
						if ($pricedfrom && $pricedto && $pricedfrom != $pricedto) {
							$items .= "<p>From {$pricedfrom} to {$pricedto}</p>";
						} elseif ($pricedfrom) {
							$items .= "<p>From {$pricedfrom}</p>";
						}

						$floorplans = project_available_floorplans($ID);
						if ($floorplans && count($floorplans) > 1) {
							$items .= '<p><i class="fa fa-lg fa-check-circle"></i> ' . count($floorplans) . " Floor Plans Available</p>";
						} elseif ($floorplans && count($floorplans) == 1) {
							$items .= '<p><i class="fa fa-lg fa-check-circle"></i> ' . count($floorplans) . " Floor Plan Available</p>";
						} else {
							$items .= '<p><i class="fa fa-lg fa-minus-circle"></i> Floor Plans coming soon</p>';
						}
					}
