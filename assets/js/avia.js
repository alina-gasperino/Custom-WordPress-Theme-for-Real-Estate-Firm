(function($) {
    ...
    $(document).ready(function() {
        ...
        window.searchForm = new $.AviaAjaxSearch({
            scope: '#map-search',
            delay: 100,
            minChars: 0,
            for_map: true,
    ...
    $.AviaAjaxSearch = function(options) {
        // console.log('init $.AviaAjaxSearch instance:')
        // console.log({
        //     options
        // })
        var defaults = {
    ...
    $.AviaAjaxSearch.prototype = {
        bind_events: function() {
            ...
            $(document).on('click', $.proxy(this.hide_search, this));
            this.scope.on('focus', '#s', $.proxy(this.show_search, this));
            this.scope.on('submit', 'form', $.proxy(this.handle_enter, this));
            this.scope.on('click', '#clear_form', $.proxy(this.clear_form, this));
            this.scope.one('focus', '#s', $.proxy(this.try_search, this));
            // this.scope.scope.find("#s").on('load', '#s', $.proxy(this.try_search, this));
        },
        ...
        hide_search: function(e) {
            if (this.scope.find(e.target).length) {
                return;
            }
            this.scope.find('.ajax_search_response').hide();
        },

        show_search: function(e) {
            if (this.scope.find('#s').val() || !this.options.minChars) {
                this.scope.find('.ajax_search_response').show();
            }
        },

        handle_enter: function(e) {
            e.preventDefault();
            var $selected = this.scope.find('.ajax_search_response a.selected').first();
            var $first = this.scope.find('a.ajax_search_entry').first();
            var $viewall = this.scope.find('a.ajax_search_entry_view_all').first();

            if ($selected.length) {
                window.location = $selected.attr('href');
            } else if ($first.length) {
                window.location = $first.attr('href');
            } else if ($viewall.length) {
                window.location = $viewall.attr('href');
            }
        },
        ...
        try_search: function(e) {
            if (!e)
                e = {
                    currentTarget: this.scope.find("#s")[0]
                }

            var code = e.keyCode || e.which;

            if (!e.currentTarget.value.length && this.options.minChars) {
                $.proxy(this.clear_form(), this);
                return;
            } else {
                this.scope.find('#clear_form').show();
            }
            ...
            if (code == 38 || code == 40) {
                e.preventDefault();
                $.proxy(this.move_selected(code), this);
                return;
            }
            ...
            if (e.currentTarget.value.length >= this.options.minChars && (this.lastVal != $.trim(e.currentTarget.value) || this.options.minChars == 0)) {
        ...
        move_selected: function(code) {

            var entries = this.scope.find('.ajax_search_response a.ajax_search_entry');
            ...
            if (code == 40) {
                this.selected = (this.selected < entries.length) ? this.selected + 1 : entries.length;
            }

            entries.removeClass('selected');
            entries.eq(this.selected - 1).addClass('selected');

        },

        clear_form: function(e) {
            this.scope.find('#s').val('');
            this.scope.find('.ajax_search_response').remove();
            this.scope.find('#clear_form').hide();
            this.selected = 0;
        },
        ...
        do_search: function(e) {
            ...
            var obj = this,
                ...
                loading = $('<div class="ajax_load"><i class="fa fa-fw fa-spin fa-spinner"></i></div>'),
                action = form.attr('action'),
                values = form.serialize();


            values += `&action=avia_ajax_search`;
            values += this.options.for_map ? '&for_map=true' : '';
            ...
            this.promise.done(function(response) {
                ...
                // results.addClass('add-previous-results');
                // console.log('results ready + add-previous-results')
            });
            ...
            // new google.maps.LatLng(43.60359581629954, -79.93012224648436),
            // new google.maps.LatLng(43.93778824520336, -78.55683123085936)
            // ),
