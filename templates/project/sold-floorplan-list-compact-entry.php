...
if ($floorplan['availability'] == 'Sold Out') {
    $availability = 'available';
} elseif ($floorplan['availability'] == 'Available') {
...
<tr class="floorplan compact <?php echo $availability ?>" <?php echo html_data_attributes($data_attributes) ?>
    ...
    <td class="floorplan__title">
        ...
        <b class="like-button"><i class="icon" aria-hidden="true"></i>&nbsp;0</b>
    </td>
...
<!--                    </pre>-->
                ...
                <?php else: ?>
                    ...
                    ?>
                    <?php if ($diff < 0): ?>
                        <div class="last-price-change"><span><?=$human_time_diff?> ago</span>&nbsp;&nbsp;<b
                                    class="diff negative">-$<?= number_format(abs($diff)) ?></b></div>
                    <?php elseif ($diff > 0): ?>
                        <div class="last-price-change"><span><?=$human_time_diff?> ago</span>&nbsp;&nbsp;<b
                                    class="diff positive">+$<?= number_format(abs($diff)) ?></b></div>
                    <?php elseif ($diff == 0): ?>
                        <div class="last-price-change"><b class="diff day-1-price">Day 1 Price</b></div>
                    <?php endif ?>
                <?php endif ?>
            <?php endif ?>
        <?php else: ?>
